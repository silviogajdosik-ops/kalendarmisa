...
├── data-godina-B.json      Liturgijska godina B (55 dana): datumi/nazivi/boje gotovi, SVE reference
│                           čitanja popunjene, tekstovi popunjeni za 8/55 dana (Došašće+Božićno vrijeme)
│                           - JOŠ NIJE upisan u data-index.json, vidi dolje
...
- **Kostur Godine B** (21.7.2026.): `data-godina-B.json` sadrži svih 55 dana (29.11.2026. - 21.11.2027.) s ispravnim datumima/nazivima/bojama/rangom, generiran skriptom `generiraj_godinu_b.py` (algoritamski računa pokretne blagdane - Uskrs preko Meeus/Jones/Butcher formule - i samotestira se protiv već poznatih datuma Godine A prije nego što generira Godinu B; svi ključni datumi ručno unakrsno provjereni i protiv gcatholic.org). Datoteka **namjerno NIJE** upisana u `data-index.json` niti u `service-worker.js` cache popis dok se ne popuni - tek tada je "objavi" (upiši u oba mjesta + bump verzije, vjerojatno 1.2.0).
- **Godina B - reference i tekstovi u tijeku** (ažurirano 21.7.2026.): sve **55/55 reference** čitanja (`citanja.*.referenca`) popunjene, sastavljene prema Katoličkom lekcionaru (1998 USA Edition, Felix Just S.J. - catholic-resources.org) i unakrsno provjerene protiv gcatholic.org za raspored nedjelja; posebna pažnja posvećena hrvatskoj razlici od američke prakse - Tijelovo ostaje na četvrtak (nije premješteno na nedjelju), pa se 9. nedjelja kroz godinu slavi odmah iza (30.5.2027.), za razliku od američkog rasporeda gdje bi ta nedjelja bila "izostavljena". Tekstovi (Šarić prijevod) popunjeni za **8/55 dana** (sve 4 nedjelje došašća + Božić/Sveta Obitelj/2. nedjelja po Božiću/Krštenje - potonja tri preuzeta izravno iz Godine A jer imaju identična ABC čitanja, samo je Krštenje dobilo novo evanđelje Mk 1,7-11 za godinu B). Preostaje 47 dana - vidi "Sljedeći koraci".
...
3. **Godina B - reference gotove, tekstovi djelomično** (ažurirano 21.7.2026.): `data-godina-B.json` ima svih 55 dana s popunjenim referencama, a tekstovi (Šarić prijevod) su gotovi za 8/55 dana (Došašće + Božićno vrijeme). Datoteka JOŠ NIJE upisana u `data-index.json` (namjerno - da se ne prikazuju prazni/nepotpuni dani u appu prije nego što se sve popuni). Sljedeći korak: popuniti preostalih 47 dana pa upisati u `data-index.json` + `service-worker.js` cache popis.
...
## Verzioniranje (trenutno na verziji 1.1.1, ažurirano 21.7.2026.)
...
1. **Dovršiti Godinu B**: reference su gotove (55/55), tekstovi gotovi za 8/55 dana (Došašće+Božić). Preostaje popuniti tekstove (`citanja.*.tekst/naslov/pripjev`) za preostalih 47 dana iz Šarić (PD) prijevoda (dohvat s eBible.org/hrv - pripazi na oblik URL-a: većina knjiga 2-znamenkasti broj poglavlja s nulom sprijeda za jednoznamenkaste brojeve npr. `MRK01.htm`, ali Psalmi imaju 3-znamenkasti oblik npr. `PSA080.htm`). Logičan redoslijed sljedećih krugova: (a) kroz godinu prije korizme + korizma (17.1.-21.3.2027.), (b) vazmeno vrijeme + Duhovi/Trojstvo/Tijelovo (28.3.-27.5.2027.), (c) kroz godinu do kraja (30.5.-21.11.2027., uključujući Veliku Gospu i Svi Sveti - big preuzeti izravno iz Godine A jer su to identična ABC čitanja). Kad je sve popunjeno: upisati `data-godina-B.json` u `data-index.json`, dodati ga u `DATOTEKE_ZA_PREDMEMORIJU` u `service-worker.js`, i objaviti kao verziju 1.2.0 (uz `provjeri.py` prije pusha).
