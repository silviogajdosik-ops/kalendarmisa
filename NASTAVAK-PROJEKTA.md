...
1. **Popuniti tekstove čitanja za preostalih 26 "backfill" dana** (2026-02-01 do 2026-07-19, kronološki u prošlosti) - glavni preostali posao, nije hitno za crkvenu upotrebu unaprijed. Raditi kronološki, u serijama od 5-10 dana po sesiji. Postupak: dohvatiti odgovarajuća poglavlja Šarić (PD) prijevoda s eBible.org/hrv, izdvojiti retke prema referenci, upisati u `tekst`/`pripjev`. "Molitva vjernika" se i dalje namjerno preskače (ostaje prazna).
2. **Nakon svakog unosa**: validirati JSON (`python -m json.tool data-godina-A.json`), bump `CACHE_NAME` u `service-worker.js` (sljedeći je v8), commit + push (git ISKLJUČIVO preko Windows PowerShell alata - vidi `GIT-UPUTE-ZA-CLAUDEA.md`).
